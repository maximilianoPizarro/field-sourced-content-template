...
The same CDC pattern implemented in this workshop applies directly to mission-critical scenarios in the oil and gas industry, where IoT sensors and SCADA systems generate continuous data that Kafka and Camel process in real time.
...
. In parallel, the pipeline persists all events (normal and anomalous) in a *Data Lake* (S3/ODF) for historical analysis and ML model training.
...
* https://docs.redhat.com/en/documentation/openshift_container_platform/4.21[Red Hat OpenShift Container Platform^] — Edge and cloud deployment platform
