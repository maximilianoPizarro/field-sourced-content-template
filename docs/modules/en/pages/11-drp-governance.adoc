...
Two mechanisms back up Kafka data:
...
Apicurio stores schemas in Kafka (`kafkasql` mode), so Kafka replicates them with the cluster.
...
** **MirrorSourceConnector** — consumes messages from the source and produces them to the target. MirrorMaker2 renames topics with the source alias as a prefix (e.g. `source.cdc.public.customers`).
...
For `compact` topics, tombstone records (key + null value) let you remove specific records when GDPR requires the right to be forgotten.
...
* https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/backup_and_restore/index[OpenShift Backup and Restore^] — OpenShift backup strategies
