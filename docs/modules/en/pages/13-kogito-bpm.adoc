...
| Camel processes CDC events linearly
...
You define the customer onboarding process as a `SonataFlow` CR using the Serverless Workflow specification:
...
. The engine runs the transitions defined in the workflow in an *event-driven* way: it evaluates and executes each state in order, using the previous result as input.
...
. The Data Index persists the instance with status `WAITING`.
...
. The engine merges the results of both branches into the instance state data.
...
Design SonataFlow workflows visually with the **Kaoto** extension in **Red Hat OpenShift Dev Spaces**:
...
6. Kaoto generates the YAML automatically
