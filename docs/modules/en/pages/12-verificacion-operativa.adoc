...
Use this page to verify the operational state of the full CDC pipeline on the OpenShift cluster. Each section includes screenshots and the YAML that defines the configuration.

== General Status of the `kafka-cdc` Namespace

The cluster runs all CDC pipeline components in the `kafka-cdc` namespace:
...
OpenShift mounts the volume at `/opt/app-root/src/postgresql-cfg` so the Red Hat PostgreSQL 16 image applies it automatically.
...
* https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/cli_tools/oc-cli-getting-started[OpenShift CLI (oc)^] — Commands for operational verification
