...
- name: s2i-build-{{ .component.component_name }}
  ...
  - name: CHAINED_BUILD_DOCKERFILE
    value: {{ .component.chained_build_dockerfile | quote }}
  {{- end }}
  - name: TAG
    value: $(tasks.bump-build-version-{{ .component.component_name }}.results.image-tag)
  - name: OUTPUT_IMAGE
    value: image-registry.openshift-image-registry.svc:5000/industrial-edge-tst-all/{{ .component.output_image_name }}
