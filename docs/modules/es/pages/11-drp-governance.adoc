...
Dos mecanismos respaldan los datos de Kafka:
...
Apicurio almacena los schemas en Kafka (`kafkasql` mode), y Kafka los replica junto con el cluster.
...
** **MirrorSourceConnector** — consume mensajes del source y los produce al target. MirrorMaker2 renombra los topics con el alias del source como prefijo (ej: `source.cdc.public.customers`).
...
. Resultado: Kafka elimina el dato por completo, sin recrear el topic.
...
* Podés agregar campos opcionales
* No podés eliminar campos requeridos
* No podés cambiar tipos de datos
...
Para cumplir con regulaciones como GDPR, configurá eliminación automática:
...
Para topics con `compact`, los tombstone records (key + null value) permiten eliminar registros específicos cuando GDPR exige el "right to be forgotten".
...
* https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/backup_and_restore/index[OpenShift Backup and Restore^] — Estrategias de backup para OpenShift
