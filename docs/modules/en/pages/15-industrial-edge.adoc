= Industrial Edge Pattern
:toc: left
:toclevels: 3

++++
<div id="scene-ie" style="width:100%;min-height:420px;"></div>
<script src="https://unpkg.com/three@0.128.0/build/three.min.js"></script>
<script src="https://unpkg.com/three@0.128.0/examples/js/controls/OrbitControls.js"></script>
<script src="../_/js/three-helper.js"></script>
<script>
(function(){
  var ctx = ThreeHelper.initScene('scene-ie', {height:420, camX:4, camY:14, camZ:30, rotateSpeed:0.35});
  if(!ctx) return;

  // Far Edge — Factory Floor
  var s1 = ThreeHelper.createNode(ctx.scene,'Sensor 1',0x607D8B,-18,0,-4);
  var s2 = ThreeHelper.createNode(ctx.scene,'Sensor 2',0x607D8B,-18,0,0);
  var s3 = ThreeHelper.createNode(ctx.scene,'Sensor 3',0x607D8B,-18,0,4);
  var amq = ThreeHelper.createNode(ctx.scene,'AMQ Broker\nMQTT',0xCC0000,-11,0,0,{width:3});

  // Near Edge — Factory DC
  var kafkaF = ThreeHelper.createNode(ctx.scene,'Kafka\nFactory',0x231F20,-4,0,-3,{width:3});
  var camelMqtt = ThreeHelper.createNode(ctx.scene,'Camel\nMQTT\u2192Kafka',0xFF6D00,-4,0,3,{width:3});
  var iot = ThreeHelper.createNode(ctx.scene,'IoT\nConsumer',0x9C27B0,2,0,-3);
  var dash = ThreeHelper.createNode(ctx.scene,'Line\nDashboard',0x42A5F5,2,0,3);

  // Core DC
  var kafkaDL = ThreeHelper.createNode(ctx.scene,'Kafka\nData Lake',0x231F20,9,0,0,{width:3});
  var mm2 = ThreeHelper.createNode(ctx.scene,'Mirror\nMaker2',0x1A73E8,9,3,4);
  var camelS3 = ThreeHelper.createNode(ctx.scene,'Camel\nKafka\u2192S3',0xFF6D00,15,0,-3,{width:3});
  var minio = ThreeHelper.createNode(ctx.scene,'MinIO S3\nData Lake',0x43A047,21,0,-3,{width:3});
  var mlserv = ThreeHelper.createNode(ctx.scene,'KServe\nAnomaly ML',0x0068B5,9,0,-5,{width:3});
  var oai = ThreeHelper.createNode(ctx.scene,'OpenShift AI\nJupyter',0xEE0000,15,0,3,{width:3});

  var edges = [
    ThreeHelper.createEdge(ctx.scene,s1,amq,0x607D8B,{speed:0.010}),
    ThreeHelper.createEdge(ctx.scene,s2,amq,0x607D8B,{speed:0.009}),
    ThreeHelper.createEdge(ctx.scene,s3,amq,0x607D8B,{speed:0.011}),
    ThreeHelper.createEdge(ctx.scene,amq,camelMqtt,0xCC0000,{speed:0.008}),
    ThreeHelper.createEdge(ctx.scene,camelMqtt,kafkaF,0xFF6D00,{speed:0.008}),
    ThreeHelper.createEdge(ctx.scene,amq,iot,0xCC0000,{speed:0.007}),
    ThreeHelper.createEdge(ctx.scene,amq,dash,0xCC0000,{speed:0.006}),
    ThreeHelper.createEdge(ctx.scene,iot,mlserv,0x9C27B0,{speed:0.007}),
    ThreeHelper.createEdge(ctx.scene,kafkaF,kafkaDL,0x231F20,{speed:0.006}),
    ThreeHelper.createEdge(ctx.scene,kafkaDL,mm2,0x1A73E8,{speed:0.005}),
    ThreeHelper.createEdge(ctx.scene,kafkaDL,camelS3,0x231F20,{speed:0.007}),
    ThreeHelper.createEdge(ctx.scene,camelS3,minio,0xFF6D00,{speed:0.008}),
    ThreeHelper.createEdge(ctx.scene,minio,oai,0x43A047,{speed:0.006})
  ];
  ThreeHelper.animate(ctx, edges);
})();
</script>
++++

[.lead]
*Use Case:* Boosting manufacturing efficiency and product quality with artificial intelligence/machine learning (AI/ML) out to the edge of the network.

[cols="1,1,1", options="header"]
|===
| Validation status | Based on | Links

| image:https://img.shields.io/badge/status-maintained-green[Maintained]
| https://validatedpatterns.io/patterns/industrial-edge/[Red Hat Validated Patterns^]
| https://github.com/maximilianoPizarro/field-sourced-content-template[GitHub^] · https://maximilianopizarro.github.io/field-sourced-content-template[Docs^]
|===

== Background

Microcontrollers and other simple computers have long been used in factories and processing plants to monitor and control machinery in modern manufacturing. The industry has consistently leveraged technology to drive innovation, optimize production, and improve operations.

Supervisory Control and Data Acquisition (SCADA) systems have historically functioned independently of a company's IT infrastructure. However, businesses increasingly recognize the value of integrating operational technology (OT) with IT. This integration enhances factory system flexibility and enables the adoption of advanced technologies such as AI and machine learning. As a result, tasks like maintenance can be scheduled based on real-time data rather than rigid schedules, while computing power is brought closer to the source of data generation.

== Solution Overview

image::ROOT:edge-manufacturing-overview.png[Industrial Edge solution overview,width=100%,align=center]

_Figure 1. Industrial edge solution overview. It is applicable across a number of verticals including manufacturing._

This solution:

* Provides *real-time insights* from the edge to the core datacenter
* Secures *GitOps and DevOps* management across core and factory sites
* Provides *AI/ML tools* that can reduce maintenance costs
* Enables *Change Data Capture* (CDC) for real-time data synchronization
* Offers *Mirror/Replicas* for external queries without impacting production

Different roles within an organization have different concerns and areas of focus when working with this distributed AI/ML architecture across two logical types of sites:

* *The core datacenter.* Data scientists, developers, and operations personnel apply changes to models, application code, and configurations here.
* *The factories.* GitOps deploys new applications, updates, and operational changes here to improve quality and efficiency in the factory.

== Logical Diagrams

image::ROOT:industrial-edge-logical.png[Industrial Edge logical architecture,width=100%,align=center]

_Figure 2. Industrial Edge solution as logically and physically distributed across multiple sites._

=== Overall Data Flows

image::ROOT:edge-mfg-data-flow.png[Overall data flows,width=100%,align=center]

_Figure 3. Overall data flows of the solution._

Two major data flow streams:

. *Northbound (edge → core):* Sensor data and events flow from the operational edge to the core for centralized processing. High volumes (tens of thousands of events per second) may make cloud transfer impractical.

. *Southbound (core → edge):* GitOps pushes code, configurations, master data, and ML models from the core to the edge. With potentially hundreds of plants and thousands of production lines, automation and consistency are essential.

== The Technology

[cols="1,3", options="header"]
|===
| Technology | Description

| https://www.redhat.com/en/technologies/cloud-computing/openshift[Red Hat OpenShift^]
| Enterprise-ready Kubernetes container platform built for an open hybrid cloud strategy. Provides a consistent application platform to manage hybrid cloud, public cloud, and edge deployments.

| https://www.redhat.com/en/products/application-foundations[Red Hat Application Foundations^]
| Frameworks and capabilities for designing, building, deploying, connecting, securing, and scaling cloud-native applications. Includes Apache Camel, AMQ, and data streaming components.

| https://www.redhat.com/en/technologies/jboss-middleware/amq[Red Hat AMQ^]
| Massively scalable, distributed, and high-performance data streaming platform based on Apache Kafka. Offers a distributed backbone for microservices and IoT applications with high throughput and low latency.

| https://www.redhat.com/en/technologies/cloud-computing/openshift/openshift-ai[Red Hat OpenShift AI^]
| Flexible, scalable AI/ML platform that enables enterprises to create and deliver AI-enabled applications at scale across hybrid cloud environments. Includes JupyterLab, ModelMesh, and Data Science Pipelines.

| https://www.redhat.com/en/technologies/management/advanced-cluster-management[Red Hat Advanced Cluster Management^]
| Controls clusters and applications from a single console, with built-in security policies. Deploys applications, manages multiple clusters, and enforces policies at scale.

| https://developers.redhat.com/rhdh[Red Hat Developer Hub^]
| Self-service developer portal based on Backstage. Provides software templates, TechDocs, and integrated plugins for Kafka, Topology, and CI/CD.

| https://strimzi.io/[Strimzi^]
| Kubernetes operator for Apache Kafka. Manages Kafka clusters, topics, users, MirrorMaker2, and KafkaConnect in a declarative way using Custom Resources.

| https://debezium.io/[Debezium^]
| Open-source CDC platform. Captures row-level changes from databases (PostgreSQL, MySQL, SQL Server) and publishes them as events to Kafka.
|===

== Architectures

=== Edge Manufacturing with Messaging and ML

image::ROOT:edge-mfg-messaging-ml.png[Edge manufacturing with messaging and ML,width=100%,align=center]

_Figure 4. Industrial Edge solution showing messaging and ML components schematically._

Sensors transmit data via *MQTT* to Red Hat AMQ, which routes it for two purposes:

* *Model development* in the core data center (data lake → JupyterLab → training)
* *Live inference* at the factory data centers (IoT Consumer → ModelMesh → anomaly alerts)

*Apache Camel (Quarkus)* provides MQTT integration to normalize and route sensor data to Kafka and then to S3 (MinIO) for the data lake. Data scientists use *OpenShift AI* tools to develop, train, and deploy models.

=== Edge Manufacturing with GitOps

image::ROOT:edge-mfg-gitops.png[GitOps workflows,width=100%,align=center]

_Figure 5. Industrial Edge solution showing a schematic view of the GitOps workflows._

*GitOps* provides a consistent, declarative approach to managing cluster changes and upgrades across centralized and edge sites. Changes to configuration and applications are automatically pushed into operational systems at the factory via *ArgoCD*.

=== Change Data Capture (CDC)

CDC with *Debezium* captures changes from relational databases and publishes them as events to a dedicated Kafka cluster. This enables:

* Event sourcing and view materialization
* Microservice synchronization
* Real-time auditing and compliance
* Analytics and search indexing

At 20K device scale, the CDC component handles ~5,000 DB transactions/sec with 5 consumer groups (Camel, notifications, search indexer, analytics, audit logger).

=== Mirror/Replicas for External Queries

*KafkaMirrorMaker2* replicates CDC and IoT topics to a read-only mirror cluster for external access:

* Analytics/BI consumers reading data without affecting production latency
* Geographic replication for teams in other regions
* Disaster recovery cluster with up-to-date data
* Secure external access (TLS + SCRAM-SHA-512) without exposing internal clusters

At 20K scale, the mirror handles 13K msg/sec from 3 source clusters with 7 external consumer groups.

== Demo Scenario

image::ROOT:highleveldemo.png[High-level demo scenario,width=100%,align=center]

_Figure 6. High-level demo summary showing machine condition monitoring based on sensor data._

The demo scenario has three layers:

. *Line Data Server (far edge):* Machine sensors on the shop floor publishing vibration and temperature readings via MQTT every 5 seconds.

. *Factory Data Center (near edge):* AMQ Broker receives MQTT data. Camel Quarkus bridges MQTT→Kafka. Factory Kafka streams data. IoT Consumer feeds ModelMesh for anomaly detection. Line Dashboard provides real-time visualization.

. *Central Data Center (core):* Kafka data lake stores mirrored data. Camel Quarkus writes to MinIO S3. OpenShift AI provides JupyterLab notebooks, Data Science Pipelines, and ModelMesh serving.

=== What's Running

[cols="1,2,1", options="header"]
|===
| Component | What it does | Namespace

| Machine Sensors (x4)
| Simulate vibration and temperature readings every 5 seconds via MQTT
| `industrial-edge-stormshift-machine-sensor`, `industrial-edge-tst-all`

| AMQ Broker (x2)
| MQTT broker receiving sensor data at the factory edge
| `industrial-edge-stormshift-messaging`, `industrial-edge-tst-all`

| Kafka Clusters (x3)
| Dev, Factory, and Data Lake clusters for event streaming
| `industrial-edge-tst-all`, `industrial-edge-stormshift-messaging`, `industrial-edge-data-lake`

| Camel Quarkus
| MQTT→Kafka bridge and Kafka→S3 data lake integration
| `industrial-edge-tst-all`, `industrial-edge-stormshift-messaging`, `industrial-edge-data-lake`

| Line Dashboard (x2)
| Real-time visualization of sensor data and anomaly alerts
| `industrial-edge-tst-all`, `industrial-edge-stormshift-line-dashboard`

| MinIO S3
| Object storage for the data lake (model training data, artifacts)
| `industrial-edge-ml-workspace`

| OpenShift AI
| JupyterLab + ModelMesh + Data Science Pipelines
| `ml-development`

| IoT Consumer (messaging)
| Backend that consumes MQTT sensor data and forwards to ML inference
| `industrial-edge-stormshift-messaging`, `industrial-edge-tst-all`
|===

== Demo Script

To explore the Industrial Edge deployment, follow the showroom guide:

. *xref:12-verificacion-operativa.adoc[Observe IoT Sensors]* — Watch sensor data flow in real-time
. *Explore Kafka Streams* — Inspect 3 Kafka clusters, topics, and live messages
. *Line Dashboard & Anomalies* — See ML anomaly detection live
. *GitOps Config Change* — Enable temperature sensors via ConfigMap
. *MinIO Data Lake* — Browse stored sensor data in S3

== Screenshots

image::ROOT:industrialedge.png[Industrial Edge — Overview,width=100%]

image::ROOT:industrialedge1.png[Industrial Edge — IoT Sensors,width=100%]

image::ROOT:industrialedge2.png[Industrial Edge — AMQ Broker,width=100%]

image::ROOT:industrialedge3.png[Industrial Edge — Kafka Streams,width=100%]

image::ROOT:industrialedge4.png[Industrial Edge — Line Dashboard,width=100%]

image::ROOT:industrialedge5.png[Industrial Edge — Camel Quarkus,width=100%]

image::ROOT:industrialedge6.png[Industrial Edge — MinIO Data Lake,width=100%]

image::ROOT:industrialedge7.png[Industrial Edge — OpenShift AI,width=100%]

image::ROOT:industrialedge8.png[Industrial Edge — Anomaly Detection,width=100%]

image::ROOT:industrialedge9.png[Industrial Edge — GitOps ArgoCD,width=100%]

image::ROOT:industrialedge10.png[Industrial Edge — Full Architecture,width=100%]

image::ROOT:streams-multicluster.png[Kafka Streams — Multi-cluster Architecture,width=100%]

image::ROOT:camel.png[Kaoto — Visual Camel Route Designer,width=100%]

== References

* https://validatedpatterns.io/patterns/industrial-edge/[Red Hat Validated Patterns — Industrial Edge^]
* https://github.com/validatedpatterns/industrial-edge[Validated Patterns GitHub Repository^]
* https://github.com/sa-mw-dach/manuela[MANUela (Manufacturing Edge Lightweight Accelerator)^]
