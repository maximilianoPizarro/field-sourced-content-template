...
Developer Hub carga el plugin como dynamic plugin:
...
El plugin de Kafka en Developer Hub no usa APIs REST de terceros — conecta *directamente* al cluster Kafka:
...
Developer Hub monta el frontend de forma condicional en las entity pages:
...
. Si la anotación existe, Developer Hub renderiza el card `EntityKafkaContent` en el `mountPoint` configurado (`entity.page.overview/cards`).
...
Abrí: `https://kafka-console-kafka-cdc.{cluster_domain}`
