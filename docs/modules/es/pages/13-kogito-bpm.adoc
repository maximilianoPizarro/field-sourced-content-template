...
| Camel procesa los eventos CDC de forma lineal
...
Instalá el operador desde Red Hat Operators:
...
El chart Helm configura la plataforma en el namespace `kogito-bpm`:
...
Definís el proceso de onboarding de clientes como un `SonataFlow` CR usando la especificación Serverless Workflow:
...
El workflow SonataFlow integra los componentes existentes del pipeline CDC:
...
SonataFlow ejecuta workflows definidos en la https://serverlessworkflow.io/[Serverless Workflow Specification] como microservicios Quarkus:

. El operador recibe el CR `SonataFlow` y genera un *Quarkus application* con el runtime del workflow embebido.
. Al recibir un evento CDC (CloudEvent via Kafka o HTTP), el motor crea una *instancia del workflow* con un ID único.
. Cada instancia mantiene su propio *estado* (state data) — un JSON document que el motor transforma en cada transición.
. El motor ejecuta las transiciones definidas en el workflow de forma *event-driven*: evalúa y ejecuta cada estado en orden, usando el resultado del anterior como input.

=== Callback pattern: aprobaciones asíncronas

El estado `RequestManagerApproval` usa el *callback pattern* de la especificación:

. El workflow ejecuta la *action* (enviar solicitud de aprobación al topic `bpm.approval.requests` via Kafka Bridge).
. El motor *suspende* la instancia del workflow — no consume CPU ni memoria mientras espera.
. La instancia queda persistida en el Data Index con estado `WAITING`.
. Cuando llega el evento `approvalCompleted` (un mensaje en el topic con el `workflowInstanceId` correcto), el motor *resume* la instancia desde el punto exacto donde se suspendió.
. Si el evento no llega en `PT24H` (24 horas), el timeout dispara y el workflow transiciona al siguiente estado con un flag de timeout.

=== Ejecución paralela: branches

El estado `ProvisionAccount` ejecuta *branches* en paralelo:

. El motor lanza ambas ramas (`CreateAccount` y `SendWelcome`) simultáneamente como coroutines.
. `completionType: allOf` indica que el workflow espera a que *ambas* ramas terminen antes de transicionar.
. Si una rama falla, el workflow puede reintentar o manejar el error según la política definida.
. El motor fusiona los resultados de ambas ramas en el state data de la instancia.
...
* Historial de transiciones (qué estados ejecutó el motor y cuándo)
...
Diseñá workflows SonataFlow visualmente con la extensión **Kaoto** en **Red Hat OpenShift DevSpaces**:
...
6. Kaoto genera el YAML automáticamente
...
* https://docs.redhat.com/en/documentation/red_hat_openshift_serverless[Red Hat OpenShift Serverless^] — Plataforma serverless que incluye Serverless Logic
* https://sonataflow.org/serverlessworkflow/latest/index.html[SonataFlow Documentation^] — Motor de workflows cloud-native
* https://serverlessworkflow.io/[Serverless Workflow Specification^] — Estándar abierto para definición de workflows
* https://kogito.kie.org/[Kogito^] — Plataforma de automatización de procesos cloud-native
* https://docs.redhat.com/en/documentation/red_hat_openshift_dev_spaces[Red Hat OpenShift Dev Spaces^] — IDE cloud-native para desarrollo de workflows con Kaoto
* https://kaoto.io/docs/[Kaoto Documentation^] — Diseñador visual para workflows y rutas Camel
