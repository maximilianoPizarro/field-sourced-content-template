...
Usá esta página para verificar el estado operativo del pipeline CDC completo en el cluster OpenShift. Cada sección incluye capturas de pantalla y el YAML que define la configuración.

== Estado General del Namespace `kafka-cdc`

El cluster corre todos los componentes del pipeline CDC en el namespace `kafka-cdc`:
...
OpenShift monta el volumen en `/opt/app-root/src/postgresql-cfg` para que la imagen Red Hat PostgreSQL 16 lo aplique automáticamente.
...
* https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/cli_tools/oc-cli-getting-started[OpenShift CLI (oc)^] — Comandos para verificación operativa
