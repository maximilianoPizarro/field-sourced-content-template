...
The Helm chart configures each CDC pipeline topic with *3 partitions* and *3 replicas*:
...
| *Replication*
| With RF=3 and `min.insync.replicas=2`, Kafka tolerates the loss of 1 broker without data loss

| *Ordering*
| Debezium uses the primary key as the partition key, preserving per-record ordering

| *Scalability*
| You can increase partitions without downtime (only added, never removed)
|===
...
* Compaction preserves the current state of each record
* Compaction removes intermediate updates
...
* `min.insync.replicas: 2` — Kafka acknowledges a write only if 2 of the 3 replicas persist it
* `default.replication.factor: 3` — Kafka creates all new topics with RF=3
...
. The producer sends the batch to the partition *leader* broker over TCP.
...
. If a consumer disconnects (no heartbeat within `session.timeout.ms`), the coordinator triggers a *rebalance* and reassigns orphaned partitions to the remaining consumers.
...
. The log cleaner physically removes messages with duplicate keys and lower offsets from disk.
...
`NetworkPolicy` rules protect the `kafka-cdc` namespace and restrict ingress traffic:
...
* https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/nodes/controlling-pod-placement-onto-nodes-scheduling[Pod Scheduling and Disruption Budgets^] — PDBs and scheduling on OpenShift
