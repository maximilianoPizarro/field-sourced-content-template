...
El chart Helm configura cada topic del pipeline CDC con *3 particiones* y *3 réplicas*:
...
| *Replicación*
| Con RF=3 y `min.insync.replicas=2`, Kafka tolera la pérdida de 1 broker sin perder datos

| *Orden*
| Debezium usa la primary key como partition key, garantizando orden por registro

| *Escalabilidad*
| Podés aumentar particiones sin downtime (solo las agregás, nunca las eliminás)
|===
...
* La compactación preserva el estado actual de cada registro
* La compactación elimina updates intermedios
* Permite reconstruir el estado completo de una tabla consumiendo el topic desde el inicio

=== Reprocesamiento

Para reprocesar eventos, reseteá el offset del consumer group:
...
WARNING: Esto hace que el consumer reprocese todos los mensajes del topic. Usá con precaución en producción.
...
* `default.replication.factor: 3` — Kafka crea todos los topics nuevos con RF=3
...
. El producer envía el batch al broker *leader* de la partición via TCP.
. El leader escribe el batch en un *segment file* en disco — un append-only log. Kafka rota los segments al llegar a 1GB (configurable).
...
. Si un consumer se desconecta (no envía heartbeat en `session.timeout.ms`), el coordinator dispara un *rebalance* y reasigna las particiones huérfanas a los consumers restantes.
...
. El log cleaner elimina físicamente del disco los mensajes con key duplicada y offset menor.
...
Las reglas `NetworkPolicy` protegen el namespace `kafka-cdc` y restringen el tráfico de ingress:
...
* https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/nodes/controlling-pod-placement-onto-nodes-scheduling[Pod Scheduling and Disruption Budgets^] — PDBs y scheduling en OpenShift
